...
static int select_jeita_cv(void)
{
	...
	if (g_temp_status == TEMP_ABOVE_POS_60)
		...
		cv_voltage = p_bat_charging_data->jeita_temp_pos_15_to_pos_45_cv_voltage;
	else if (g_temp_status == TEMP_POS_0_TO_POS_15)
		cv_voltage = p_bat_charging_data->jeita_temp_pos_0_to_pos_15_cv_voltage;
	else if (g_temp_status == TEMP_BELOW_ZERO)
		cv_voltage = p_bat_charging_data->jeita_temp_below_0_cv_voltage;
	else
...
int do_jeita_state_machine(void)
{
	...
	if (BMT_status.temperature >= p_bat_charging_data->temp_pos_60_threshold) {
	...
	} else if (BMT_status.temperature >= p_bat_charging_data->temp_pos_15_threshold) {
		if (((g_temp_status == TEMP_POS_45_TO_POS_60)
			...
			&& (BMT_status.temperature <=
			    p_bat_charging_data->temp_pos_15_thres_plus_x_degree))) {
			battery_log(BAT_LOG_CRTI,
...
static void set_jeita_charging_current(void)
{
	...
	if ((BMT_status.charger_type == NONSTANDARD_CHARGER) || (BMT_status.charger_type == CHARGING_HOST))
		return;
...
void select_charging_curret(void)
{
	if (g_ftm_battery_flag) {
		...
		battery_log(BAT_LOG_ERROR,
			"[BATTERY] Default CC mode charging : %d, input current = %d, charger type = %d, JEITA status = %d\r\n",
			g_temp_CC_value, g_temp_input_CC_value, BMT_status.charger_type, g_temp_status);
	}
...
static void pchr_turn_on_charging(void)
{
	...
	if (BMT_status.bat_charging_state == CHR_ERROR) {
		...
		if (g_temp_CC_value == CHARGE_CURRENT_0_00_MA
			...
			cv_voltage = select_jeita_cv();
#endif
			battery_log(BAT_LOG_FULL, "[BATTERY] charging cv voltage is %d\r\n", cv_voltage);
			bat_charger_set_cv_voltage(cv_voltage);
		}
	}
...
#if defined(CONFIG_MTK_JEITA_STANDARD_SUPPORT)
		if (g_temp_status == TEMP_POS_0_TO_POS_15) {
...
#if defined(CONFIG_MTK_JEITA_STANDARD_SUPPORT)
	if ((g_temp_status == TEMP_ABOVE_POS_60) || (g_temp_status == TEMP_BELOW_ZERO))
